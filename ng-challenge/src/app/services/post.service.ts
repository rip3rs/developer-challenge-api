...
export class PostService {
  ...
  public getAll$(): Observable<Post[]> {
    return this._http.get<Post[]>(`${environment.beBaseUrl}/posts?_sort=publish_date&_order=desc`).pipe(
      take(1),
      map((res) => {
        if (res.length > 0) {
  ...
  public getOne$(slug: string): Observable<Post> {
    return this._http.get<Post[]>(`${environment.beBaseUrl}/posts/${slug}&_embed=comments`).pipe(
      take(1),
      map((post) => {
        if (post.length > 0) {
